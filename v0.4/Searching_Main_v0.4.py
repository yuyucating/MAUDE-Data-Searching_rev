...
frame_openPath = tk.Frame(window)
frame_openPath.pack(fill="x", pady=5, padx=5)
dL = tk.StringVar()
dataLink = ttk.Entry(frame_openPath, width=26, textvariable=dL)
dataLink.grid(row=0, column=0)
btn_openPath = ttk.Button(frame_openPath, text="瀏覽", width=5, command=SearchingKeyFile)
btn_openPath.grid(row=0, column=1, sticky="e", padx=2)
